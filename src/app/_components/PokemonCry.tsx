...
const PokemonCry = ({ color, link }: { color: string; link: URL }) => {
    ...
    const hexaDarkColor = `lch(from ${hexaColor} calc(l - 55) c h)`;
    ...
    return (
        <div className="flex flex-row gap-4 items-center my-4 rounded-4xl px-5 py-2" style={{ backgroundColor: hexaDarkColor }}>
            <button className="rounded-full p-5 hocus:!bg-[color:var(--darken-bg-color)]" style={{ backgroundColor: hexaColor, color: hexaLightColor }} onClick={onPlayPause} type="button">
                {isPlaying ? (
